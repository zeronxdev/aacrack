...
Get_Sys_Version(){
    ...
    if [  -z "${OS_NAME}" ] || [ -z "${X86_CHECK}" ];then
        wget -O lib.sh ${download_Url}/install/0/lib.sh && bash lib.sh 
        exit
    fi
}
...
Install_Curl()
{
	if [ ! -f "/usr/local/curl/bin/curl" ];then
		cd /usr/local
		wget -O ${OS_NAME}-${OS_V}-curl.tar.gz ${download_Url}/soft/lib/curl/${OS_NAME}-${OS_V}-curl.tar.gz -T 20
		tar -zxf ${OS_NAME}-${OS_V}-curl.tar.gz
		rm -f ${OS_NAME}-${OS_V}-curl.tar.gz
		cd ${run_path}
	fi
}
...
Install_Openssl()
{
	if [ ! -f "/usr/local/openssl/lib/libssl.so" ];then
		cd /usr/local
		wget -O ${OS_NAME}-${OS_V}-openssl102.tar.gz ${download_Url}/soft/lib/openssl/${OS_NAME}-${OS_V}-openssl102.tar.gz -T 20
		tar -zxf ${OS_NAME}-${OS_V}-openssl102.tar.gz
		rm -f ${OS_NAME}-${OS_V}-openssl102.tar.gz
		cd ${run_path}
...
Install_Libiconv()
{
	...
	if [ "${OS_V}" ];then
		cd /usr/local
		wget -O ${OS_NAME}-${OS_V}-libiconv-1.14.tar.gz ${download_Url}/soft/lib/libiconv/${OS_NAME}-${OS_V}-libiconv-1.14.tar.gz
		tar -xvf ${OS_NAME}-${OS_V}-libiconv-1.14.tar.gz
		rm -f ${OS_NAME}-${OS_V}-libiconv-1.14.tar.gz
...
Install_Libmcrypt()
{
	...
	cd ${run_path}
	if [ "${OS_V}" ];then 
		wget -O ${OS_NAME}-${OS_V}-libmcrypt-2.5.8.tar.gz ${download_Url}/soft/lib/libmcrypt/${OS_NAME}-${OS_V}-libmcrypt-2.5.8.tar.gz
		tar -xvf ${OS_NAME}-${OS_V}-libmcrypt-2.5.8.tar.gz
		rm -f ${OS_NAME}-${OS_V}-libmcrypt-2.5.8.tar.gz
		cd libmcrypt-2.5.8
...
Install_Mcrypt()
{
	...
	if [ "${OS_V}" ];then
		wget -O ${OS_NAME}-${OS_V}-mcrypt-2.6.8.tar.gz ${download_Url}/soft/lib/mcrypt/${OS_NAME}-${OS_V}-mcrypt-2.6.8.tar.gz
		tar -xvf ${OS_NAME}-${OS_V}-mcrypt-2.6.8.tar.gz
		rm -f ${OS_NAME}-${OS_V}-mcrypt-2.6.8.tar.gz
		cd mcrypt-2.6.8
		make install
		cd ${run_path}
		rm -f {OS_NAME}-${OS_V}-mcrypt-2.6.8.tar.gz
...
Install_Mhash()
{
	...
	cd ${run_path}
	if [ "${OS_V}" ];then
		wget -O ${OS_NAME}-${OS_V}-mhash-0.9.9.9.tar.gz ${download_Url}/soft/lib/mhash/${OS_NAME}-${OS_V}-mhash-0.9.9.9.tar.gz
		tar -xvf ${OS_NAME}-${OS_V}-mhash-0.9.9.9.tar.gz
		cd mhash-0.9.9.9
		make install
		cd ${run_path}
		rm -f ${OS_NAME}-${OS_V}-mhash-0.9.9.9.tar.gz
		rm -rf mhash-0.9.9.9
	else
